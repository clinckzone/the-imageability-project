...
function draw() {
  ...
  for (let i = 0; i < 14; i++) {
    ...
    let mX = mouseX - translateX;
    let mY = mouseY - ht + translateY;
    console.log(mouseX);
...
class path {
  ...
  show() {
    ...
    let radius_max = 35; //Scaling radius
    let max_value = 10; //maximum value from chart

    let rad1 = map(this.rig, 0, max_value, 0, radius_max);
    let rad2 = map(this.lef, 0, max_value, 0, radius_max);
    let rad3 = map(this.cen, 0, max_value, 0, radius_max);
    let rad4 = map(this.rd, 0, max_value, 0, radius_max);

    let dif = max(rad1, rad2);
    let dis = this.len;
    let padding = 5;
    // let diff = min(this.len-dif,)
    let x1 = dis - dif / 2 - rad3 / 2;
    let y1 = rad1 / 2 + padding;
    let x2 = dis - dif / 2 - rad3 / 2;
    let y2 = -rad2 / 2 - padding;
    let x3 = dis - dif - rad3 / 2;
    let y3 = 0;
    let x4 = dis;
    let y4 = 0;
